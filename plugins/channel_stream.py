...
# এখানে CHANNEL_FILE_CAPTION যোগ করা হয়েছে
from info import BIN_CHANNEL, URL, CHANNEL, IS_SHORTLINK, TUTORIAL_LINK_1, CHANNEL_FILE_CAPTION
...
async def channel_receive_handler(bot: Client, broadcast: Message):
    try:
        ...
        raw_stream = f"{base_url}watch/{msg.id}/avbotz.mkv?hash={get_hash(msg)}"
        raw_download = f"{base_url}{msg.id}?hash={get_hash(msg)}"
        ...
        # Using the variable from info.py
...
async def group_link_handler(bot: Client, message: Message):
    try:
        ...
        raw_stream = f"{base_url}watch/{log_msg.id}/avbotz.mkv?hash={get_hash(log_msg)}"
        raw_download = f"{base_url}{log_msg.id}?hash={get_hash(log_msg)}"
